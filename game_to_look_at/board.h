...
class moveBoard{
	...
	moveBoard(char c, int xs, int ys, int xe, int ye): mP(c), xi(xs), yi(ys), xf(xe), yf(ye) {}
...
class board{
	...
	static void convertCommand(const std::string&, Driver &driver, Checkerboard &chk);
...
//-------------------------------------------------------------------------------------
	...
	void inputCommand(Driver &driver, Checkerboard &chk);
	...
	void printMoves(Driver &driver, Checkerboard &chk);

	//makes the move
	//should be used on a copy of a board when alpha-beta searching
	void makeMove(moveBoard*);

	//reverses a move
	void undoMove(moveBoard*);
